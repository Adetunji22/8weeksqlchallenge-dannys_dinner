 ...
 select * from temp where rank=1
  ...
  from(
   ...
   10. 	  /*In the first week after a customer joins the program (including their join date) 
	...
	WITH temp AS (
		select *
			   , (join_date + INTERVAL '6 days')::date as new_date
			   , (DATE_TRUNC('month', join_date) + INTERVAL '1 month - 1 day')::date as EOMONTH
		from members m
		)
select customer_id 
       , SUM(CASE
		WHEN  product_name = 'sushi'
		 THEN price * 20
		WHEN order_date BETWEEN join_date AND new_date
		 THEN price * 20
		ELSE price * 10
	 END) points
from temp t
...
join menu me
USING(product_id)
where order_date <= EOMONTH
GROUP BY customer_id
ORDER BY customer_id


                     -- BONUS QUESTIONS
  1.  --JOINING ALL TABLES
